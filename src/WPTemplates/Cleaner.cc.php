...
function CleanerGenerate($wpName, $LHS, $RHS, $queries, $queryNames) {

    $lhs = $LHS;
    $rhs = $RHS;
    //$lhs = array_map('lookupAttribute', $LHS);
    //$rhs = array_map('lookupAttribute', $RHS);
?>
...
extern "C"
int CleanerWorkFunc_<?=$wpName?>( WorkDescription & workDescription, ExecEngineData & result ) {
    int counter = 0;
    ...
    for (; myWork.get_theseQueriesAreDone ().RightLength (); myWork.get_theseQueriesAreDone ().Advance ()) {
        ...
        for (myWork.get_equivalences ().MoveToStart (); ; myWork.get_equivalences ().Advance ()) {
            ...
            if (!(myWork.get_equivalences ().RightLength ())) {
                std::cout << "Cound not find a match for ";
    ...
    std::vector<Column> hashColumnLHS(numDyingWaypoints);
    std::vector<ColumnIterator <HT_INDEX_TYPE> > hashColumnIterLHS(numDyingWaypoints);

    // now is the bitmap column
    //Column bitmapColumnLHS[numDyingWaypoints];
    std::vector<BStringIterator> bitmapColumnIterLHS(numDyingWaypoints);
...
<?  foreach( $lhs as $att ) {
        ...
        $attType = $att->type();
?>
    std::vector<Column> <?=$attName?>_Column_LHS(numDyingWaypoints);
    std::vector< <?=$attType->iterator()?> > <?=$attName?>_ColumnIter_LHS(numDyingWaypoints);
    int <?=$attName?>_Column_LHSIsUsed[numDyingWaypoints];
<?  } // foreach LHS attribute ?>
    ...
    for (int i = 0; i < numDyingWaypoints; i++) {
        ...
        Bitstring tempBits(<?=$queries?>, true);
...
<?  foreach( $lhs as $att ) { ?>
        MMappedStorage <?=$att->name()?>_Column_Store;
        Column <?=$att?>_Col( <?=$att?>_Column_Store);
        <?=$att->type()->iterator()?> <?=$att?>_ColumnIter(<?=$att?>_Col);
        <?=$att?>_ColumnIter.swap(<?=$att?>_ColumnIter_LHS[i]);
        <?=$att?>_Column_LHSIsUsed[i] = 0;
<?  } // foreach LHS attribute ?>
    }

    // RHS IS NEXT
    // first is the hash column
    std::vector<Column> hashColumnRHS(numDyingWaypoints);
    std::vector<ColumnIterator <HT_INDEX_TYPE> > hashColumnIterRHS(numDyingWaypoints);

    // now is the bitmap column
    std::vector<Column> bitmapColumnRHS(numDyingWaypoints);
    std::vector<BStringIterator> bitmapColumnIterRHS(numDyingWaypoints);
    ...
    // IN THE ENTIRE SYSTEM
<?  foreach( $rhs as $att ) { ?>
    std::vector<Column> <?=$att?>_Column_RHS(numDyingWaypoints);
    std::vector<<?=$att->type()->iterator()?> > <?=$att?>_ColumnIter_RHS(numDyingWaypoints);
    int <?=$att?>_Column_RHSIsUsed[numDyingWaypoints];
<?  } // foreach RHS attribute ?>
...
<?  foreach( $rhs as $att ) { ?>
        ...
        Column <?=$att?>_Col( <?=$att?>_Column_Store);
        <?=$att->type()->iterator()?> <?=$att?>_ColumnIter(<?=$att?>_Col);
        <?=$att?>_ColumnIter.swap(<?=$att?>_ColumnIter_RHS[i]);
        <?=$att?>_Column_RHSIsUsed[i] = 0;
<?  } // foreach RHS attribute ?>
    }
...
<?  foreach( $lhs as $att ) { ?>
        <?=$att?>_ColumnIter_LHS[IDX].MarkFragment ();
<?  } // foreach LHS attribute ?>
<?  foreach( $rhs as $att ) { ?>
        <?=$att?>_ColumnIter_RHS[IDX].MarkFragment ();
<?  } // foreach RHS attribute ?>
    }
...
<?  foreach( $lhs as $att ) { ?>
            ...
            if (lastLen > 0 && state == DYING_AND_SEND && LHS) {
                ...
                for (; <?=$att?>_Column_LHSIsUsed[index] < bitmapColumnLHSIsUsed[index] - 1; <?=$att?>_Column_LHSIsUsed[index]++) {
                    <?=$att->type()?> col; // TBD, some default value
                    <?=$att?>_ColumnIter_LHS[index].Insert (col);
                    <?=$att?>_ColumnIter_LHS[index].Advance ();
                }

                // Now put the data in
                <?=$att->type()?> *<?=$att?>_ColumnPtr = ((<?=$att->type()?>*) serializeHere);
                <?=$att?>_ColumnIter_LHS[index].Insert(*<?=$att?>_ColumnPtr);
                <?=$att?>_ColumnIter_LHS[index].Advance();
                <?=$att?>_Column_LHSIsUsed[index]++;

            } else if( lastLen > 0 && state == DYING_AND_SEND && !LHS ) {
                FATAL("<?=$att?> is only used by the LHS, how did I find it in a RHS tuple?");
...
<?  foreach( $rhs as $att ) { ?>
            ...
            // see if we got something... in the first two cases, we are extracting data for a disk-based join
            if (lastLen > 0 && state == DYING_AND_SEND && LHS) {
                FATAL("<?=$att?> is only used by the RHS, how did I find it in a LHS tuple?");
            } else if( lastLen > 0 && state == DYING_AND_SEND && !LHS ) {
                // first, make sure that we are far enough along
                for (; <?=$att?>_Column_RHSIsUsed[index] < bitmapColumnRHSIsUsed[index] - 1; <?=$att?>_Column_RHSIsUsed[index]++) {
                    <?=$att->type()?> col; // TBD, some default value
                    <?=$att?>_ColumnIter_RHS[index].Insert (col);
                    <?=$att?>_ColumnIter_RHS[index].Advance ();
                }

                // Now put the data in
                <?=$att->type()?> *<?=$att?>_ColumnPtr = ((<?=$att->type()?>*) serializeHere);
                <?=$att?>_ColumnIter_RHS[index].Insert(*<?=$att?>_ColumnPtr);
                <?=$att?>_ColumnIter_RHS[index].Advance();
                <?=$att?>_Column_RHSIsUsed[index]++;
            } else if( lastLen > 0 && !bitstringPtr->IsEmpty() && state == ALIVE ) {
                // in the last case, the data will go back into the hash table
                storage.Append (columnID, serializeHere, lastLen);
            }
...
<?  foreach( $lhs as $att ) { ?>
            if( <?=$att?>_Column_LHSIsUsed[i] ) {
                <?=$att->type()?> colDummyVal;

                // first, make sure that we are far enough along
                for( ; <?=$att?>_Column_LHSIsUsed[i] < bitmapColumnLHSIsUsed[i]; <?=$att?>_Column_LHSIsUsed[i]++)
                {
                    <?=$att?>_ColumnIter_LHS[i].Insert( colDummyVal );
                    <?=$att?>_ColumnIter_LHS[i].Advance();
                }

                Column col;
                <?=$att?>_ColumnIter_LHS[i].Done(col);
                myChunk.SwapColumn(col, <?=$att->slot()?>);
            }
<?  } // foreach LHS attribute ?>
    ...
    for (int i = 0; i < numDyingWaypoints; i++) {
        ...
        if (bitmapColumnRHSIsUsed[i]) {
            ...
            // now add the atts, one-at-a-time, if valid
<?  foreach( $rhs as $att ) { ?>
            if( <?=$att?>_Column_RHSIsUsed[i] ) {
                <?=$att->type()?> colDummyVal;

                // first, make sure that we are far enough along
                for( ; <?=$att?>_Column_RHSIsUsed[i] < bitmapColumnRHSIsUsed[i]; <?=$att?>_Column_RHSIsUsed[i]++)
                {
                    <?=$att?>_ColumnIter_RHS[i].Insert( colDummyVal );
                    <?=$att?>_ColumnIter_RHS[i].Advance();
                }

                Column col;
                <?=$att?>_ColumnIter_RHS[i].Done(col);
                myChunk.SwapColumn(col, <?=$att->slot()?>);
            }
<?  } // foreach RHS attribute ?>
